...
\begin{proof}
    ...
    Solve this equation and find a solution of $f$, \mn{This is known as Stein transform.}
    ...
    By completing the square\mn{Notice that we have two square term $x^2/2,y^2/2$ here so this is a good thing to try.}, we see that $e^{-y^2} \leq e^{-x^2/2}e^{-x(y-x)}$. 
...
\begin{theorem}
    [Berry-Esseen theorem, less weak form.]
...
\begin{proof}
    ...
    By the fundamental theorem of calculus, we have \mn{I don't understand where this $t$ comes from.}
    \begin{equation*}
    ...
    Since $X_i$ and $Z_{n;i}$ are independent and meanzero, the first term on the rhs vanishes. Thus \mn{It seems that here he used some stochastic calculus technique, I don't understand why could one put a uniformly random $t$ here and have the following claim.}
    \begin{equation}
...
This restores the exponent in factor on \ref{Weak to general berry esseen} from $1/4$ to $1/2$. 

We can push this argument even further to when the $X_i$ is not only bounded in third moment, but actually bounded.

\begin{theorem}
    [Berry-Esseen theorem, bounded case]
...
Taking expectations, using the independence of $X_n$ and $Z_{n-1}$ and the normalisation, we obtain

\begin{equation*}
    \E \varphi(Z_n)=\E \varphi(\frac{\sqrt{n-1}}{\sqrt{n}}Z_{n-1})+\frac{1}{2n}\varphi''(\frac{\sqrt{n-1}}{\sqrt{n}}Z_{n-1})+O(\frac{1}{n^{3/2}})
\end{equation*}

Up to errors of $O(\frac{1}{n^{3/2}})$,\mn{Why the approximation on the second order can have error of the third order.} one can approximate the second term ere by $\frac{1}{2n}\varphi''(Z_{n-1})$. We then insert this into
\begin{equation*}
    \varphi(Z_n) \approx \E \varphi (Z_{n-1})\approx \int_\R  \varphi(x)\rho(x) \rmd x
\end{equation*}
...
which leads us to the heuristic ODE\mn{This is like magic. How can someone first came up with this???}
\begin{equation*}
    L\rho(x)=0
\end{equation*}
